...
from coRegistration.login.models import SSHCredentials
...
def remoteHost_Setup_and_upload(currentUserName= None, hostname=None, username=None, password=None):
	...
	print("currentUserRemoteIn %s " % currentUserRemoteIn)
	print("currentUserRemoteOut %s " % currentUserRemoteOut)
...
def setup_configuration_files(currentUserLocalDir=None, imageSrc1_remotePath = None, imageSrc2_remotePath= None, imageRfr_remotePath=None):
	...
	print("imageEPSG %d " % imageEPSG)
	print("imageSrc1_remotePath %s " % imageSrc1_remotePath)
...
def	remote_run_routine_and_save(currentUserName= None, hostname=None, username=None, password=None):
	...
	try:
		...
		logger.debug("utils.py: function remote_run_routine_and_save: 2")
...
def cleanUp(currentUserName= None, hostname=None, username=None, password=None):

	logger.debug("utils.py: function cleanUp: 1")
	UseGSSAPI = True             # enable GSS-API / SSPI authentication
	DoGSSAPIKeyExchange = True
	port = 22
	# now, connect and use paramiko Client to negotiate SSH2 across the connection
	try:
		...
		usersRemoteHomePath = roothPath + 'users/'
		folderTobedeleted = "cd " + usersRemoteHomePath + " ; rm -r " + currentUserName + "/"
		print("folderTobedeleted %s " % folderTobedeleted)
		
		logger.debug("utils.py: function cleanUp: 2")
		stdin, stdout, stderr = client.exec_command(folderTobedeleted)

		logger.debug("utils.py: function cleanUp: 3")
...
# 		rmtree(sftp, currentUsrRemoteHomePath)
# 		logger.debug("utils.py: function cleanUp: 3")
